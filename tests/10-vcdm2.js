...
describe("Verifiable Credentials Data Model v2.0", function () {
  ...
  for (const [name, implementation] of match) {
    ...
    describe(name, function () {
      ...
      it.skip(
        'Conforming document (compliance): VCDM "MUST be enforced." ' +
          '("all relevant normative statements in Sections 4. Basic Concepts, ' +
          '5. Advanced Concepts, and 6. Syntaxes")',
        async function () {}
      );
      // Basic
      it.skip(
      ...
      reportRow(
        "verifiers MUST produce errors when non-conforming documents " +
          "are detected.",
        async function () {
          const doc = {
      ...
      reportRow(
        ...
        async function () {
          ...
          await assert.rejects(
            endpoints.issue(require("./input/credential-no-context-fail.json"))
          );
        }
      );
      reportRow(
      ...
      reportRow(
        "Verifiable credentials: The value of the @context property " +
      ...
      reportRow(
        "Verifiable presentations: The value of the @context " +
      ...
      reportRow(
        'Verifiable credential @context: "Subsequent items in the ' +
          "array MUST be composed of any combination of URLs and/or objects " +
          'where each is processable as a JSON-LD Context."',
        async function () {
          await endpoints.issue(
            require("./input/credential-context-combo1-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-context-combo2-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-context-combo3-fail.json")
            )
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-context-combo4-fail.json")
            )
          );
        }
      );
      reportRow(
        'if present: "The id property MUST express an identifier ' +
          "that others are expected to use when expressing statements about a " +
          'specific thing identified by that identifier."',
        async function () {
          await endpoints.issue(require("./input/credential-id-other-ok.json"));
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-id-nonidentifier-fail.json")
            )
          );
        }
      );
      reportRow(
        ...
        async function () {
          await endpoints.issue(
            require("./input/credential-single-id-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-subject-single-id-ok.json")
          );
          await assert.rejects(
          ...
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-subject-multi-id-fail.json")
            )
          );
        }
      );
      reportRow(
        'if present: "The value of the id property MUST be a URL ' +
      ...
      reportRow(
        ...
        async function () {
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-nonsingle-id-fail.json")
            )
          );
        }
      );
      ...
      reportRow(
        ...
        async function () {
          await assert.rejects(
            endpoints.verifyVp(
              require("./input/presentation-no-type-fail.json")
            )
          );
        }
      );
      reportRow(
        ...
        async function () {
          ...
          await endpoints.issue(
            require("./input/credential-type-mapped-url-ok.json")
          );
          // type mapped not to URL: fail
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-type-mapped-nonurl-fail.json")
            )
          );
          // type not mapped: fail
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-type-unmapped-fail.json")
            )
          );
        }
      );
      reportRow(
        'type property: "If more than one URL is provided, the URLs ' +
          'MUST be interpreted as an unordered set."',
        async function () {
          //issue VC with multiple urls in type property
          await endpoints.issue(
            require("./input/credential-type-urls-order-1-ok.json")
          );
          //issue another VC with same urls in a different order
          await endpoints.issue(
            require("./input/credential-type-urls-order-2-ok.json")
          );
        }
      );
      ...
      reportRow(
        ...
        async function () {
          ...
          await endpoints.issue(
            require("./input/credential-optional-type-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-missing-required-type-fail.json")
            )
          );
          ...
          await assert.rejects(
            endpoints.verifyVp(
              require("./input/presentation-missing-required-type-fail.json")
            )
          );
          ...
          await assert.rejects(
            endpoints.verify(
              require("./input/credential-proof-missing-type-fail.json")
            )
          );
          await endpoints.issue(require("./input/credential-status-ok.json"));
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-status-missing-type-fail.json")
            )
          );
          await endpoints.issue(
            require("./input/credential-termsofuse-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-termsofuse-missing-type-fail.json")
            )
          );
          await endpoints.issue(require("./input/credential-evidence-ok.json"));
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-evidence-missing-type-fail.json")
            )
          );
        }
      );
      it.skip(
        "All credentials, presentations, and encapsulated objects " +
      ...
      reportRow(
        ...
        async function () {
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-subject-no-claims-fail.json")
            )
          );
          await endpoints.issue(
            require("./input/credential-subject-multiple-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-subject-multiple-empty-fail.json")
            )
          );
        }
      );
      ...
      reportRow(
        "The value of the issuer property MUST be either a URL, or " +
          "an object containing an id property whose value is a URL.",
        async function () {
          await endpoints.issue(
            require("./input/credential-issuer-object-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-url-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
          ...
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-issuer-object-no-id-fail.json")
            )
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-issuer-object-id-not-url-fail.json")
            )
          );
        }
      );
      reportRow(
        'If present, the value of the "issuer.name" property MUST be ' +
          "a string or a language value object as described in 10.1 Language and " +
          "Base Direction.",
        async function () {
          await endpoints.issue(
            require("./input/credential-issuer-name-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-name-optional-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-name-language-en-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-name-language-direction-en-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-multi-language-name-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-issuer-name-extra-prop-en-fail.json")
            )
          );
        }
      );
      reportRow(
        'If present, the value of the "issuer.description" property ' +
          "MUST be a string or a language value object as described in 10.1 " +
          "Language and Base Direction.",
        async function () {
          await endpoints.issue(
            require("./input/credential-issuer-description-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-description-optional-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-description-language-en-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-description-language-" +
              "direction-en-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-issuer-multi-language-description-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-issuer-description-extra-prop-en-fail.json")
            )
          );
        }
      );
      reportRow(
        ...
        async function () {
          await endpoints.issue(
            require("./input/credential-validfrom-ms-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-validfrom-tz-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-validfrom-invalid-fail.json")
            )
          );
        }
      );
      reportRow(
        ...
        async function () {
          ...
          await endpoints.issue(
            require("./input/credential-validuntil-ms-ok.json")
          );
          await endpoints.issue(
            require("./input/credential-validuntil-tz-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-validuntil-invalid-fail.json")
            )
          );
        }
      );
      reportRow(
        "If a validUntil value also exists, the validFrom value MUST " +
          "express a datetime that is temporally the same or earlier than the " +
          "datetime expressed by the validUntil value.",
        async function () {
          const positiveTest = require("./input/credential-validUntil-validFrom-ok.json");
          ...
          const negativeTest = require("./input/credential-validUntil-validFrom-fail.json");
      ...
      reportRow(
        "If a validFrom value also exists, the validUntil value MUST " +
      ...
      reportRow(
        "A conforming document MUST be secured by at least one " +
        ...
        async function () {
          ...
          if (Array.isArray(issuedVc.proof)) {
            issuedVc.proof.length.should.be.gt(
              0,
      ...
      reportRow(
        "A conforming issuer implementation produces conforming " +
          ...
          "Section 4.9 Securing Mechanisms.",
        async function () {
          should.exist(issuedVc, `Expected ${name} to issue a VC.`);
          should.exist(issuedVc.proof, "Expected VC to have a proof.");
        }
      ...
      reportRow(
        "A conforming verifier implementation consumes conforming " +
        ...
        async function () {
          ...
          assert.rejects(
            endpoints.verify(require("./input/credential-ok.json"))
          );
        }
      );
      // end securing mechanism block
      reportRow(
        "(If a credentialStatus property is present), The type " +
        ...
        async function () {
          ...
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-status-type-nonurl-fail.json")
            )
          );
          await endpoints.issue(require("./input/credential-status-ok.json"));
        }
      ...
      reportRow(
        "If present (credentialStatus.id), the normative guidance " +
          "in Section 4.3 Identifiers MUST be followed.",
        async function () {
          // id is optional
          await endpoints.issue(
            require("./input/credential-status-missing-id-ok.json")
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-status-multiple-id-fail.json")
            )
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-status-nonurl-id-fail.json")
            )
          );
        }
      );
      reportRow(
        "In Verifiable Presentations, the verifiableCredential " +
        ...
        async function () {
          ...
          await assert.rejects(
            endpoints.verifyVp(
              require("./input/presentation-vc-missing-required-type-fail.json")
            )
          );
          await assert.rejects(
            endpoints.verifyVp(
              require("./input/presentation-derived-vc-missing-required-type-fail.json")
            )
          );
        }
      );
      ...
      // Advanced
      reportRow(
        "JSON-LD-based processors MUST produce an error when a " +
          "JSON-LD context redefines any term in the active context.",
        async function () {
          // This depends on "@protected" (which is used for the base context).
          await assert.rejects(
            endpoints.issue(require("./input/credential-redef-type-fail.json"))
          );
          await assert.rejects(
            endpoints.issue(require("./input/credential-redef-type2-fail.json"))
          );
        }
      );
      reportRow(
      ...
      reportRow(
        "Each credentialSchema MUST specify its type (for example, " +
          "JsonSchemaValidator2018), and an id property.",
        async function () {
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-schema-no-type-fail.json")
            )
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-schema-no-id-fail.json")
            )
          );
        }
      );
      reportRow(
        ...
        async function () {
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-schema-non-url-id-fail.json")
            )
          );
        }
      );
      ...
      it.skip(
        "Each refreshService value MUST specify its type (for example, " +
          "ManualRefreshService2018) and its id, which is the URL of the " +
          "service.",
        async function () {
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-refresh-no-type-fail.json")
            )
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-refresh-no-id-fail.json")
            )
          );
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-refresh-non-url-id-fail.json")
            )
          );
        }
      );
      reportRow(
        ...
        async function () {
          await endpoints.issue(
            require("./input/credential-termsofuses-ok.json")
          );
        }
      );
      reportRow(
        "Each termsOfUse value MUST specify its type, for example, " +
          "IssuerPolicy, and MAY specify its instance id.",
        async function () {
          await assert.rejects(
            endpoints.issue(
              require("./input/credential-termsofuse-no-type-fail.json")
            )
          );
          await endpoints.issue(
            require("./input/credential-termsofuse-id-ok.json")
          );
        }
      );
      reportRow(
        ...
        async function () {
          ...
          const x = await endpoints.issue(
            require("./input/credential-evidences-ok.json")
          );

          // console.log("---- issueofDatas-----");
          // console.log(x);
      ...
      it.skip(
        "(ZKP) The verifiable credential MUST contain a Proof, using " +
      ...
      it.skip(
        "(ZKP) If a credential definition is being used, the " +
      ...
      it.skip(
        "(ZKP) Each derived verifiable credential within a " +
      ...
      it.skip(
        "A verifiable presentation MUST NOT leak information that " +
      ...
      it.skip(
        "Data model mapping property values to JSON types " +
